...
| **代码审查** | `pm-code-review` | 安全/逻辑/性能审查 |
| **SEO 策略** | `pm-seo` | SEO 审计与关键词策略 |
| **通用内容** | `pm-content-general` | Blog/Landing Page/Release Notes |
...
| Agent 编排 | 单 Agent + Skill 路由 | 多 Agent 为 v2 规划 |
...
│   │   ├── agents/                 # 5 个子 Agent（v2 规划，仅 frontmatter）
│   │   │   ├── pm-spec-writer/     # 规格撰写
│   │   │   ├── pm-prototyper/      # 原型生成
│   │   │   ├── pm-builder/         # 构建执行
│   │   │   ├── pm-researcher/      # 研究分析
...
- Windows 安装包（NSIS）
- 评估体系（eval-v3 行为正确性评估，8 个 smoke P0 + 真实场景验证）
