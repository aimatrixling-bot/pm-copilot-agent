...
面向 PM 的桌面端 AI Agent（Apache-2.0）。Tauri v2 + Claude Agent SDK，34 个 PM Skill（三轨：PM/Engineering/Growth） + 23 本书方法论 KB。使用 Conventional Commits，不提交敏感信息。
...
当前所有功能通过 pm-copilot 主 Agent + 34 个 PM Skill（三轨：PM/Engineering/Growth） 路由实现。路由表见 `bundled-agents/pm-copilot/CLAUDE.md`。
