...
本文档描述 PM Copilot 的构建流程、发布流程以及分发渠道的完整信息。

---

## 概览

PM Copilot 支持 **macOS** 和 **Windows** 平台：
...
> **Windows 用户**：请参阅 [windows_build_guide.md](./windows_build_guide.md)
...
所有发布文件存储在 **Cloudflare R2**，通过自定义域名 `download.ai-matrix.site` 提供访问。
...
└── releases/
    └── v{VERSION}/
        ├── PM Copilot_{VERSION}_aarch64.dmg         # ARM DMG
        ├── PM Copilot_{VERSION}_x64.dmg             # Intel DMG
        ├── PM Copilot_{VERSION}_aarch64.app.tar.gz  # ARM 更新包
        ├── PM Copilot_{VERSION}_aarch64.app.tar.gz.sig  # ARM 签名
        ├── PM Copilot_{VERSION}_x64.app.tar.gz      # Intel 更新包
        └── PM Copilot_{VERSION}_x64.app.tar.gz.sig  # Intel 签名
```
...
```
  ...
  ├─────────────────────────────────────────────────────────┤
  │  Apple Silicon (ARM64)                                  │
  │    ✓ DMG:    PM Copilot_0.1.0_aarch64.dmg              │
  │    ✓ tar.gz: PM Copilot.app.tar.gz                      │
  │    ✓ 签名:   PM Copilot.app.tar.gz.sig                  │
  │                                                         │
  │  Intel (x86_64)                                         │
  │    ✓ DMG:    PM Copilot_0.1.0_x64.dmg                   │
  │    ✓ tar.gz: PM Copilot.app.tar.gz                      │
  │    ✓ 签名:   PM Copilot.app.tar.gz.sig                  │
  └─────────────────────────────────────────────────────────┘
```
...
```json
{
  ...
  "release_notes": "PM Copilot v0.1.0",
  "downloads": {
    "mac_arm64": {
      "name": "Apple Silicon",
      "url": "https://download.ai-matrix.site/releases/v0.1.0/PM Copilot_0.1.0_aarch64.dmg"
    },
    "mac_intel": {
      "name": "Intel Mac",
      "url": "https://download.ai-matrix.site/releases/v0.1.0/PM Copilot_0.1.0_x64.dmg"
    }
  }
}
```
...
**官网使用示例**：
```typescript
const res = await fetch('https://download.ai-matrix.site/update/latest.json');
...
{
  "version": "0.1.0",
  "notes": "PM Copilot v0.1.0",
  "pub_date": "2026-01-24T10:00:00Z",
  "platforms": {
    "darwin-aarch64": {
      "signature": "dW50cnVzdGVkIGNvbW1lbnQ6...",
      "url": "https://download.ai-matrix.site/releases/v0.1.0/PM Copilot.app.tar.gz"
    }
  }
}
```
...
```json
{
  "plugins": {
    "updater": {
      ...
      "endpoints": [
        "https://download.ai-matrix.site/update/{{target}}.json"
      ]
    }
  }
}
...
```bash
# 检查官网 API
curl -s https://download.ai-matrix.site/update/latest.json | jq .

# 检查自动更新清单
curl -s https://download.ai-matrix.site/update/darwin-aarch64.json | jq .
```
...
```bash
npx tauri signer generate -w ~/.tauri/pm-copilot.key
```
...
**更新检查失败**
- 检查 CSP 配置是否允许 `download.ai-matrix.site`
...
- [windows_build_guide.md](./windows_build_guide.md) - Windows 版本构建流程
- [auto_update.md](./../tech_docs/auto_update.md) - 静默更新流程、CI/CD 配置
- [macos_distribution_guide.md](./macos_distribution_guide.md) - 代码签名、公证详解
- [bundled_bun.md](./../tech_docs/bundled_bun.md) - 运行时打包机制
