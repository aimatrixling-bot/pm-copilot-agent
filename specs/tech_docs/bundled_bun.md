...
PM Copilot 将 Bun 运行时打包到应用内，实现零外部依赖分发。用户无需安装 Bun 或 Node.js 即可运行应用，包括 MCP（Model Context Protocol）功能。
