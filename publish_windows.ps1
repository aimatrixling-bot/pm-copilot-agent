...
Write-Host "  PM Copilot Windows 发布到 R2" -ForegroundColor Green
...
$ManifestDir = [System.IO.Path]::GetTempPath() + "pm-copilot-manifest-" + [System.Guid]::NewGuid().ToString("N")
...
if ($UpdateZip) {
    ...
    # 重命名上传文件名，添加版本和架构标识
    $UpdateUploadName = "PM Copilot_${Version}_x86_64.nsis.zip"
...
if ($NsisExe) {
    ...
    $latestWinManifest = @{
        version       = $Version
        pub_date      = $PubDate
        release_notes = "PM Copilot v$Version"
...
if ($SigFile) {
    Write-Host "    - PM Copilot_${Version}_x86_64.nsis.zip.sig"
    $uploadFiles += $SigFile
}
...
if ($SigFile) {
    ...
    $sigUploadName = "PM Copilot_${Version}_x86_64.nsis.zip.sig"
