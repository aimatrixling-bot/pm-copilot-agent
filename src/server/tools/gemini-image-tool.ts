...
function getGeneratedDir(): string {
  ...
  const dir = workspace
    ? join(workspace, 'pm-copilot-generated', 'images')
    : join(homedir(), '.pm-copilot', 'generated');
  if (!existsSync(dir)) {
    mkdirSync(dir, { recursive: true });
    // Ensure .gitignore in pm-copilot-generated/ parent to prevent accidental commits
    if (workspace) {
      const parentDir = join(workspace, 'pm-copilot-generated');
...
function getContextsDir(): string {
  const dir = join(homedir(), '.pm-copilot', 'image-contexts');
