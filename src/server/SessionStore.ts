...
const PM_COPILOT_DIR = join(homedir(), '.pm-copilot');
const SESSIONS_FILE = join(PM_COPILOT_DIR, 'sessions.json');
const SESSIONS_DIR = join(PM_COPILOT_DIR, 'sessions');
const ATTACHMENTS_DIR = join(PM_COPILOT_DIR, 'attachments');
const SESSIONS_TMP_FILE = join(PM_COPILOT_DIR, 'sessions.json.tmp');
...
export function updateSessionMetadata(
    sessionId: string,
    updates: Partial<Pick<SessionMetadata, 'title' | 'lastActiveAt' | 'sdkSessionId' | 'unifiedSession' | 'stats' | 'source' | 'lastMessagePreview' | 'titleSource' | 'runtime' | 'runtimeSessionId'>>
