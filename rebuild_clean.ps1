...
Write-Host "║  🔧 PM Copilot 彻底清理重建                              ║" -ForegroundColor Cyan
Write-Host "╚═══════════════════════════════════════════════════════╝" -ForegroundColor Cyan
Write-Host ""
...
# 验证关键部分
$required = @("http://ipc.localhost", "asset:", "fetch-src", "https://download.pm-copilot.io")
$missing = @()
...
if (-not $SkipUninstall) {
    ...
    # 检查是否安装
    $app = Get-WmiObject -Class Win32_Product | Where-Object { $_.Name -like "PM Copilot*" } | Select-Object -First 1
...
$webviewCache = "$env:LOCALAPPDATA\PM Copilot\EBWebView"
...
$appData = "$env:APPDATA\PM Copilot"
...
Get-Process | Where-Object { $_.ProcessName -eq "PM Copilot" } | ForEach-Object {
    Stop-Process -Id $_.Id -Force -ErrorAction SilentlyContinue
    $killed++
}
...
$cleanDirs = @(
    ...
    "src-tauri\target\x86_64-pc-windows-msvc\release\build\pm-copilot-*",
