...
echo ""
echo -e "${CYAN}╔═══════════════════════════════════════════════════════╗${NC}"
echo -e "${CYAN}║${NC}  ${GREEN}🤖 PM Copilot macOS Dev 构建${NC}                           ${CYAN}║${NC}"
...
# 杀死残留 PM Copilot 实例（避免生产版和 debug 版同时运行互相打架）
# 优先使用 PID lock file 精确杀——只杀 PM Copilot 主进程，不误杀其他 bun 进程。
# SIGKILL(-9) 防止 macOS Automatic Termination 自动重启被杀的 .app。
echo -e "${BLUE}[准备] 杀死残留进程...${NC}"
LOCK_FILE="$HOME/.pm-copilot/app.lock"
if [ -f "$LOCK_FILE" ]; then
    OLD_PID=$(cat "$LOCK_FILE" 2>/dev/null)
    # Validate PID is a positive integer before using it with kill
    if [[ "$OLD_PID" =~ ^[1-9][0-9]*$ ]] && kill -0 "$OLD_PID" 2>/dev/null; then
        echo -e "${YELLOW}  杀死运行中的 PM Copilot (PID $OLD_PID)...${NC}"
        kill -9 "$OLD_PID" 2>/dev/null || true
    fi
    rm -f "$LOCK_FILE"
fi
# Fallback: 杀死任何漏网的 PM Copilot 进程（lock file 可能不存在或 PID 已过期）
pkill -9 -f "PM Copilot.app" 2>/dev/null || true
...
rm -rf "${PROJECT_DIR}/src-tauri/target/debug/PM Copilot.app"
