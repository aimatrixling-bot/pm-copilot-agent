...
fn inject_terminal_env(cmd: &mut CommandBuilder, app: &AppHandle, sidecar_port: Option<u16>) {
    ...
    // ~/.pm-copilot/bin (CLI tools)
    if let Some(home) = dirs::home_dir() {
        let cli_bin = home.join(".pm-copilot").join("bin");
        if cli_bin.exists() {
    ...
    if let Some(proxy) = crate::proxy_config::read_proxy_settings() {
        match crate::proxy_config::get_proxy_url(&proxy) {
            Ok(proxy_url) => {
                ...
                cmd.env("http_proxy", &proxy_url);
                cmd.env("https_proxy", &proxy_url);
                cmd.env("PM_COPILOT_PROXY_INJECTED", "1");
            }
            Err(e) => {
                ulog_error!(
    ...
    // 3. Sidecar port — lets `pm-copilot` CLI talk to the Tab's session sidecar
    if let Some(port) = sidecar_port {
        cmd.env("PM_COPILOT_PORT", port.to_string());
    }
    ...
    cmd.env("TERM_PROGRAM", "PM Copilot");
    ...
    // 7. Terminal indicator (so scripts can detect they're in PM Copilot terminal)
    cmd.env("PM_COPILOT_TERMINAL", "1");
}
