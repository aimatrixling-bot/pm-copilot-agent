...
//!
//! Configuration is read from `~/.pm-copilot/config.json` and can be enabled/disabled
...
/// Proxy settings from `~/.pm-copilot/config.json`
///
/// # Example JSON
/// ```json
/// {
...
/// Read proxy settings from ~/.pm-copilot/config.json
...
/// Apply PM Copilot proxy policy to a child process `Command`.
///
...
pub fn apply_to_subprocess(cmd: &mut Command) -> bool {
    if let Some(proxy_settings) = read_proxy_settings() {
        ...
        // No PM Copilot proxy configured: inherit system network behavior.
