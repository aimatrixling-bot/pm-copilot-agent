...
| # | 禁止模式 | 修正动作 |
| --- | --------- | -------- |
| HB-1 | 验收标准缺失 Given（前置条件） | 补充具体的触发状态和前置条件 |
| HB-2 | 需求描述含"适当"/"合理"/"必要时"等模糊词 | 替换为可量化的标准 |
| HB-3 | 未引用来源 PRD 中的对应功能编号 | 补充 PRD Feature ID 和 User Story 编号 |
| HB-4 | In Scope / Out Scope 边界含"视情况"/"待定" | 强制二选一，不确定的列入 Future |
| HB-5 | 工时估算无依据 | 补充估算依据（历史数据/类比/专家判断） |
