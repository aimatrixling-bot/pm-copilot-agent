...
# 获取带架构后缀的 tar.gz 文件名
# 用法: get_tar_upload_name "PM Copilot.app.tar.gz" "aarch64-apple-darwin"
# 输出: PM Copilot_aarch64.app.tar.gz
get_tar_upload_name() {
    local base="$1"
    local target="$2"
...
echo ""
echo -e "${CYAN}╔═══════════════════════════════════════════════════════╗${NC}"
echo -e "${CYAN}║${NC}  ${GREEN}🚀 PM Copilot 发布到 Cloudflare R2${NC}                     ${CYAN}║${NC}"
...
for TARGET in "${FOUND_TARGETS[@]}"; do
    ...
    # PM Copilot.app.tar.gz -> PM Copilot_aarch64.app.tar.gz 或 PM Copilot_x86_64.app.tar.gz
